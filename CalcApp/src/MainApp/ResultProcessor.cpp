...
void ProcessSeveralResults(ActionChainDefPtr chain, MainConfigPtr config, LoggerPtr logger)
{
    ...
    if (QDialog::Accepted == chooseResultDialog.exec())
    {
        ...
        ShowViewer(chain.get()->Results[selectedResultIndex], config.get()->Viewers, logger);
    }
}
...
void ResultProcessor::ShowResult(ActionChainDefPtr chain)
{
    ...
    if (chain.get()->Results.size() == 0)
        ...
        ProcessSeveralResults(chain, _config, _logger);
}
